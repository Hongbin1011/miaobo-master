package com.anzhuo.video.app.dongtu.utils.FrescoUtils;
...
import com.anzhuo.video.app.dongtu.widget.CustomProgressBar;
...
//                .setDownsampleEnabled(true)//Downsampling，它处理图片的速度比常规的裁剪更快，  先不使用，不然压缩后图片失真，目前不知道原因
...
//                               BaseControllerListener listener) {
//        url = append(url);
//        load(Uri.parse(url), draweeView, null, width, height, listener, false);
//    }
     ...
     * @param width
     * @param height
     * @param IsAutoPlay 是否自动播放gif
     * @param listener
     */
    public static void loadGifUrl(String url, SimpleDraweeView draweeView, int width, int height, boolean IsAutoPlay,
                                  BaseControllerListener listener) {
        url = append(url);
//        lowImageUrl = append(lowImageUrl);
        loadIsAutoPlayAnimations(Uri.parse(url), draweeView, null, width, height, IsAutoPlay, listener);
    }
...
//            }
    ...
    public static void load(Uri uri, @NonNull SimpleDraweeView draweeView, BasePostprocessor processor, int width, int height,
...
//            }
        ...
        ImageRequest request =
                ImageRequestBuilder.newBuilderWithSource(uri)
                        ...
                        .setProgressiveRenderingEnabled(true)//支持图片渐进式加载  主要是Jpeg
     ...
     */
    public static void load(Uri uri, @NonNull SimpleDraweeView draweeView, BasePostprocessor processor, int width, int height,
                            @Nullable BaseControllerListener listener, boolean isRetry) {
...
//            }
     ...
     * @param width
     * @param height
     * @param IsAutoPlayAnimations 是否自动播放gif
     * @param
     * @param listener
     */
    public static void loadIsAutoPlayAnimations(Uri uri, @NonNull SimpleDraweeView draweeView, BasePostprocessor processor, int width, int height, boolean IsAutoPlayAnimations,
                                                @Nullable BaseControllerListener listener) {
...
//            }
        ...
        //小图
//        ImageRequest lowImageRequest =
//                ImageRequestBuilder.newBuilderWithSource(lowImageUri)
//                        .setLocalThumbnailPreviewsEnabled(true)
//                        .setResizeOptions(resizeOptions)
//                        .setPostprocessor(processor)
//                        //缩放,在解码前修改内存中的图片大小, 配合Downsampling可以处理所有图片,否则只能处理jpg,
...
//                        .setLowResImageRequest(lowImageRequest)
                        ...
                        .setControllerListener(listener)
                        .setOldController(draweeView.getController())
                        .setAutoPlayAnimations(IsAutoPlayAnimations) //自动播放gif动画
                        .setTapToRetryEnabled(true)
                        .build();
        draweeView.setController(controller);

//        ImageRequest request2 = ImageRequest.fromUri(lowImageUri);
//        ImageRequest request3 = ImageRequest.fromUri(lowImageUri);
//        ImageRequest[] requestss = {request2,request3};
//        PipelineDraweeController build = (PipelineDraweeController) Fresco.newDraweeControllerBuilder()
//                .setFirstAvailableImageRequests(requestss)
//                .setImageRequest(request)
//                .setOldController(draweeView.getController())
//                .build();
//        draweeView.setController(build);
     ...
     * 表示单张图片请求
     *
     * @param ShowLongPicture
     * @return
     */
    public static GenericDraweeHierarchy GetHierarchy(boolean ShowLongPicture) {
        return GetHierarchy(true, ShowLongPicture);
    }
    ...
    public static GenericDraweeHierarchy GetHierarchy(boolean GetHierarchy, boolean ShowLongPicture) {
        ScalingUtils.ScaleType scaleType = null;//缩放类型
        PointF focusPoint = null;//缩放点
      CustomProgressBar ProgressDrawable = null;//进度条
        //单张图片
        if (GetHierarchy) {
            ProgressDrawable = new CustomProgressBar();
            if (ShowLongPicture) {//显示长图
                scaleType = ScalingUtils.ScaleType.FOCUS_CROP;
                focusPoint = new PointF(0f, 0f);
            } else {
                scaleType = ScalingUtils.ScaleType.FIT_XY;
                //设置进度条的颜色
                ProgressDrawable.setColor(VideoApplication.getContext().getResources().getColor(R.color.video_main));
                //设置背景颜色
                ProgressDrawable.setBackgroundColor(VideoApplication.getContext().getResources().getColor(R.color.black));
                //设置进度条高度
                ProgressDrawable.setBarWidth(5);
            }
        } else {//9宫格图片
        ...
        GenericDraweeHierarchyBuilder builder =
                new GenericDraweeHierarchyBuilder(VideoApplication.getContext().getResources());
        GenericDraweeHierarchy hierarchy = builder
                .setActualImageScaleType(scaleType)
                .setActualImageFocusPoint(focusPoint)
                .setProgressBarImage(ProgressDrawable)//设置加载进度条
//                .setPlaceholderImage(R.drawable.refresh_loding)//加载中
//                .setPlaceholderImage(R.drawable.gif_icon)//加载中
...
*/
 ...
 /*   public static void clearCacheByUrl(String url) {
     ...
     * @return
     */
   /* public static boolean copyCacheFile(Context context, String url, File dir, String fileName) {
        url = append(url);
        File path = new File(dir, fileName);
        return copyCacheFile(context, url, path);
    }*/
     ...
     * @return
     */
   /* public static boolean copyCacheFile(Context context, String url, File path) {
        if (path == null) {
            return false;
        }
     ...
     * @return
     */
   /* public static File copyCacheFileToDir(Context context, String url, File dir) {
        url = append(url);
        if (dir == null) {
            return null;
        }
...
*/
  ...
  /*  public static boolean isCached(Context context, String url) {
     ...
     */
 /*   public static void download(final String url, final Context context, final File dir, final DownloadListener listener) {
        ...
        dataSource.subscribe(new BaseDataSubscriber<Void>() {
            ...
            protected void onNewResultImpl(DataSource<Void> dataSource) {
                ...
                File file = copyCacheFileToDir(context, url, dir);
...
//     * @param listener
//     */
  /*  public static void getBitmapWithProcessor(@NonNull final String url, @NonNull final Context context, @NonNull final int width, @NonNull final int height,
        ...
        dataSource.subscribe(new BaseBitmapDataSubscriber() {
            ...
            protected void onNewResultImpl(Bitmap bitmap) {
                ...
                if (bitmap == null) {
                    File cacheFile = getFileFromDiskCache(context, finalUrl);
                    //还要判断文件是不是gif格式的
...
//假设最多有5张脸
    ...
    private static String getRealType(File file) {
        ...
        try {
            ...
            String type = VideoFrescoUtil.bytesToHexString(b).toUpperCase();
     ...
     * @param listener
     */
    public static void loadVideoImgUrl(String url, SimpleDraweeView draweeView, int width, int height,
                                       BaseControllerListener listener) {
        url = append(url);
        loadVideoImg(Uri.parse(url), draweeView, null, width, height, listener);
    }

    public static void loadVideoImg(Uri uri, @NonNull SimpleDraweeView draweeView, BasePostprocessor processor, int width, int height,
                                    @Nullable BaseControllerListener listener) {
        if (width == 0 && height == 0) {//如果外部没有传入,那么通过draweeView拿到控件的宽高
            measureView(draweeView);
            height = draweeView.getMeasuredHeight();
...
//            }
        ...
        ImageRequest request =
                ImageRequestBuilder.newBuilderWithSource(uri)
                        ...
                        //缩放,在解码前修改内存中的图片大小, 配合Downsampling可以处理所有图片,否则只能处理jpg,
                        // 开启Downsampling:在初始化时设置.setDownsampleEnabled(true)
                        .setProgressiveRenderingEnabled(false)//支持图片渐进式加载  主要是Jpeg
                        .setRotationOptions(RotationOptions.autoRotate()) //如果图片是侧着,可以自动旋转
                        .build();
