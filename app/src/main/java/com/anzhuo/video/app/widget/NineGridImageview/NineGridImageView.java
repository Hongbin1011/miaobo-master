...
public class NineGridImageView<T> extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mImgDataList != null && mImgDataList.size() > 0) {
            ...
            } else if (mImgDataList.size() == 2 || mImgDataList.size() == 4) {
                mImageViewList.get(0).setScaleType(SimpleDraweeView.ScaleType.CENTER_CROP);
                mGridSize = ((totalWidth / 3) * 2 - mGap * (mColumnCount - 1)) / mColumnCount;
            } else {
                mImageViewList.get(0).setScaleType(SimpleDraweeView.ScaleType.CENTER_CROP);
    ...
    private void layoutChildrenView() {
        ...
        for (int i = 0; i < childrenCount; i++) {
           GridImageView childrenView = (GridImageView) getChildAt(i);
            if (mAdapter != null) {
                mAdapter.onDisplayImage( i, childrenView, t);
            }
     ...
     * @param lists 图片数据集合
     */
    public void setImagesData(List lists,T t) {
    ...
    private SimpleDraweeView getImageView(final int position) {
        if (position < mImageViewList.size()) {
            ...
            if (mAdapter != null) {
                SimpleDraweeView SimpleDraweeView = mAdapter.generateImageView();
                mImageViewList.add(SimpleDraweeView);
                SimpleDraweeView.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        mAdapter.onItemImageClick( t, position);
                    }
                });
