package com.anzhuo.video.app.dongtu.ui.adapter;
...
import com.anzhuo.video.app.dongtu.bean.dongtu.DongtuUserClickCommentBean;
...
import com.anzhuo.video.app.dongtu.utils.MyTextUtils;
...
public class RefreshAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof ItemViewHolder) {
            ...
            Logger.i("cq=============[没有更多数据了 adapter]===========" + mLoadMoreStatus);
...
//                    break;
    ...
    public int getItemCount() {
        ...
        if (mDatas.size() == 0) {
            ...
            Logger.i("cq=============[size大小3]===========" + mDatas.size());
    ...
    public void AddHeaderItem(List<CommentBean> items) {
        ...
        Logger.i("cq=============[当前的状态]===========" + mDatas.toString());
    ...
    public void AddFooterItem(List<CommentBean> items) {
        mDatas.addAll(items);
        Logger.i("cq=============[当前的状态 的数据 刷新]===========" + mDatas.size());
        notifyDataSetChanged();
    }
    ...
    private void setData(ItemViewHolder holder, final CommentBean contentListBean, final int positionss) {
        ...
        Logger.i("cq=============[动图设置数据 这里 id]===========" + contentListBean.toString());
        List<DongtuUserClickCommentBean> states2 = new ArrayList<DongtuUserClickCommentBean>();
//        Logger.i("cq=============[设置数据 这里 id]===========" + contentListBean.getId().toString());
//        states2.clear();
        if (!TextUtils.isEmpty(contentListBean.getId().toString())) {
            states2.add(where("itemid=? and dongtuId = ? ", contentListBean.getItemid(), contentListBean.getId()).findFirst(DongtuUserClickCommentBean.class));
            Logger.i("cq=============[设置数据 qian ]===========" + states2.toString());
    ...
    private void CommentZan(final CommentBean contentListBean, final TextView tvItemZan) {
        ...
        if (XUtilNet.isNetConnected()) {
            List<DongtuUserClickCommentBean> itemids = new ArrayList<DongtuUserClickCommentBean>();
            Logger.i("cq=============[data 评论点赞]===========" + itemids.toString());
            itemids.add(where("itemid=? and dongtuId = ? ", mId, contentListBean.getId()).findFirst(DongtuUserClickCommentBean.class));
            if (itemids.get(0) != null) {
                DongtuToastUtil.showCenterToast("你已经点赞");
            } else {
                //评论点赞上报
                HomeModel.getCommentZan(contentListBean.getId(), "0", new NewInterface() {
                    @Override
                    public void onError(Call call, Exception e) {
                        DongtuToastUtil.showCenterToast("点赞失败");
                    }

                    @Override
                    public void onSucceed(int state, String data, JSONObject obj) {
                        if (state == 200) {
                            Logger.i("cq=============[data 评论点赞]===========" + obj.toString());
                            boolean save = new DongtuUserClickCommentBean()
                                    .setZan(true)
                                    .setCai(false)
    ...
    private void UpComment(CommentBean contentListBean) {
        if (XUtilNet.isNetConnected()) {
            HomeModel.getReport(contentListBean.getId(), new NewInterface() {
                @Override
                public void onError(Call call, Exception e) {
                    DongtuToastUtil.showCenterToast("举报失败");
                }

                @Override
                public void onSucceed(int state, String data, JSONObject obj) {
                    if (state == 200) {
                        DongtuToastUtil.showCenterToast("举报成功");
                    }
                }
            });
        } else {
