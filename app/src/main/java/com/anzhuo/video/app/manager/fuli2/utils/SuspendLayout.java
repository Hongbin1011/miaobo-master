package com.anzhuo.video.app.manager.fuli2.utils;
...
public class SuspendLayout extends RelativeLayout {
    ...
    public void reDraw(){
        int mY=mDragView.getTop();
        if(mDragView.getTop()<0){
            mY=0;
        }
        if(mDragView.getBottom()>getHeight()){
            mY=getHeight()-mDragView.getMeasuredHeight();
        }
        if(mDragView.getRight()-mDragView.getMeasuredWidth()/2>getWidth()/2){
            initPointPosition.x= (int) (getWidth()-mDragView.getWidth()*mPercent);
            initPointPosition.y=mY;
