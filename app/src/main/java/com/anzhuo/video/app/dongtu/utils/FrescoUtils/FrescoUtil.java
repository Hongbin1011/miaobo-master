...
//    }
     ...
     * @param listener
     */
    public static void loadGifUrl(String url, String lowImageUrl, SimpleDraweeView draweeView, int width, int height, boolean IsAutoPlay,
                                  BaseControllerListener listener) {
        url = append(url);
        lowImageUrl = append(lowImageUrl);
        loadIsAutoPlayAnimations(Uri.parse(url), Uri.parse(lowImageUrl), draweeView, null, width, height, IsAutoPlay, listener);
    }
...
//            }
     ...
     * @param listener
     */
    public static void loadIsAutoPlayAnimations(Uri uri, Uri lowImageUri, @NonNull SimpleDraweeView draweeView, BasePostprocessor processor, int width, int height, boolean IsAutoPlayAnimations,
                                                @Nullable BaseControllerListener listener) {
    ...
    public static GenericDraweeHierarchy GetHierarchy(boolean GetHierarchy, boolean ShowLongPicture) {
        ...
        if (GetHierarchy) {
            ...
            if (ShowLongPicture) {//显示长图
                ...
                //设置进度条的颜色
                ProgressDrawable.setColor(VideoApplication.getContext().getResources().getColor(R.color.red));
                //设置背景颜色
                ProgressDrawable.setBackgroundColor(VideoApplication.getContext().getResources().getColor(R.color.green));
                //设置进度条高度
                ProgressDrawable.setBarWidth(10);
            }
        } else {//9宫格图片
